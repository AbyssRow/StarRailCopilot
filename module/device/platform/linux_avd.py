...
class LinuxAVDLifecycle:
    ...
    def stop(self):
        ...
        # ADB emu kill is safe only while the exact configured AVD process is
        # currently visible. A remembered Popen object alone is not ownership
        # proof because its PID may have been reused by another AVD.
        owns_process = bool(self.matching_processes())
    ...
    def _force_stop(self):
        ...
        """
        if not _POSIX_PROCESS_CONTROL:
            return self._force_stop_without_process_groups()
    ...
    def _force_stop_without_process_groups(self):
        """Use process methods on platforms without POSIX process groups."""
        force_wait = min(max(self.settings.stop_timeout / 2, self.poll_interval), 10.0)
        for method, warning, success in (
                ('terminate', 'Linux AVD shutdown timed out; terminating exact process',
                 'Linux AVD stopped after terminate'),
                ('kill', 'Linux AVD terminate timed out; killing exact process',
                 'Linux AVD stopped after kill'),
        ):
            self._logger.warning(warning)
            self._signal_matching_processes(None, method)
            if self._wait_for_shutdown(self._monotonic() + force_wait):
                return self._finish_shutdown(success)
    ...
    def _signal_matching_processes(self, signum, method):
        ...
        if _POSIX_PROCESS_CONTROL and self._matching_process_uses_group(processes, process_group):
            try:
